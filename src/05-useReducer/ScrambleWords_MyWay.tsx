...
export const ScrambleWords = () => {
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if (guess === currentWord) {
      ...
      const nextIndex = skipCounter + points + 1;
      ...
      const nextError = errorCounter + 1;
      setErrorCounter(nextError);
      setGuess('');
      if (nextError >= maxAllowErrors) {
        setIsGameOver(true);
      }
    }
  };
  ...
  const handlePlayAgain = () => {
    ...
    const newIndex = 0;
    const newCurrentWord = newShuffledWords[newIndex];
